...
import { buildAbsenceCacheTag, invalidateAbsenceAiCaches } from './ai-validation.cache';
import { authorizeAbsenceAiJob, parseAbsenceAiJob } from './ai-validation.auth';
import {
    type AbsenceAiValidationJob,
    type AbsenceAiValidationResult,
    type AbsenceAiValidationServiceDeps,
...
export class AbsenceAiValidationService {
    ...
    handle(payload: unknown): ResultAsync<AbsenceAiValidationResult, Error> {
        return parseAbsenceAiJob(payload).andThen((parsed) =>
            authorizeAbsenceAiJob(parsed).andThen(({ authorization }) =>
                this.processJob(parsed, authorization),
            ),
        );
    }
    ...
    private processJob(
        ...
        return this.loadAbsence(parsed, authorization)
            ...
            .andThen(({ absence, attachment }) => this.loadType(absence, parsed).map((absenceType) => ({ absence, attachment, absenceType })))
    ...
    private fetchAttachment(attachment: AbsenceAttachment, authorization: RepositoryAuthorizationContext) {
    ...
    private persist(
        ...
        parsed: AbsenceAiValidationJob,
        authorization: RepositoryAuthorizationContext,
    ): ResultAsync<{ absence: UnplannedAbsence; aiResult: AbsenceDocumentAiValidatorResult; authorization: RepositoryAuthorizationContext }, Error> {
        const updatedMetadata = mutateAbsenceMetadata(absence.metadata, (store) => {
            const aiMetadata = isAiValidationMetadata(store.aiValidation)
                ? { ...store.aiValidation }
                : {};
            store.aiValidation = {
                ...aiMetadata,
...
function isAiValidationMetadata(value: unknown): value is AbsenceAiValidationMetadata {
