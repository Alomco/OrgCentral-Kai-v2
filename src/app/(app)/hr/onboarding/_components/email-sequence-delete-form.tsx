...
import { deleteEmailSequenceTemplateAction } from '../actions';
...
export interface EmailSequenceDeleteFormProps {
    templateId: string;
}

export function EmailSequenceDeleteForm({ templateId }: EmailSequenceDeleteFormProps) {
    const [state, formAction, pending] = useActionState(
        async (_previousState: ActionState, formData: FormData) => deleteEmailSequenceTemplateAction(formData),
        initialState,
    );
        ...
        <form ref={formReference} action={formAction} className="inline-flex flex-col items-end gap-1" aria-busy={pending}>
            ...
            <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This deactivates the sequence. Existing invitations may still reference it.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
            ...
            {state.status === 'error' ? (
                <div
                    ...
                    aria-atomic="true"
                >
                    {state.message ?? 'Unable to delete sequence.'}
                </div>
            ) : null}
        </form>
    );
