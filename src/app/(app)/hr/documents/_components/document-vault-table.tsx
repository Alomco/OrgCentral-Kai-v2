...
export function DocumentVaultTable({ documents }: DocumentVaultTableProps) {
    if (documents.length === 0) {
        return <p className="text-sm text-muted-foreground">No documents stored yet.</p>;
    }
                ...
                <TableBody>
                    {documents.map((document_) => (
                        <TableRow key={document_.id}>
                            ...
                            <TableCell>
                                <Link
                                    href={`/api/hr/documents/${document_.id}/download`}
